...
import { LEASE, FICTION, INTRO_CARDS, SUMMARY } from './lease-data.js';
...
function buildDocument() {
  ...
  hotspots().forEach((h, i) => {
    h.rects.forEach((r, ri) => {
      ...
      if (ri === 0) {
        b.dataset.primary = '1';
        const ref = h.paragraphRef ? `, paragraph ${h.paragraphRef.replace(/¶/g, '').trim()}` : '';
...
function renderIntro() {
  const f = FICTION;
  els.privacyNotice.textContent =
    `The names, addresses, dates, dollar amounts and other identifying information shown in ` +
    `this educational example are fictionalized. The resident is ${f.residentName}, the ` +
    `community is ${f.propertyName} at ${f.propertyAddress}, ${f.propertyCity}, GA ` +
    `${f.propertyZip}, and the monthly rent is ${f.monthlyRent}. None of these values, and no ` +
    `value anywhere in these pages, refers to a real resident or a real property. The fictional ` +
    `values are part of the page images themselves — nothing is covered over in the browser.`;

  const host = els.precards;
  host.textContent = '';
  INTRO_CARDS.forEach((card, ci) => {
    const c = el('section', 'precard' + (ci === 0 ? ' precard--num' : ''));
    c.appendChild(el('h2', null, card.title));
    c.appendChild(el('p', 'precard__note', card.note));
    const list = el(ci === 0 ? 'ol' : 'ul');
    card.items.forEach(([t, d]) => {
      const li = el('li');
...
function cache() {
  const ids = [
    ...
    'prevPage', 'nextPage', 'prevTerm', 'nextTerm', 'termCount', 'fullPageBtn',
    ...
    'startBtn', 'reviewBtn', 'privacyNotice', 'precards', 'summaryGrid', 'introBtn',
  ];
